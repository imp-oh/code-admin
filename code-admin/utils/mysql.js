 ...
 */
let { getProperty } = require('@/utils/dot-prop')
...
const handlePage = (page = {}) => {
  let { pageNum = 1, pageSize = 10 } = page
  // return pageSize ? [(Number(pageNum) - 1) * Number(pageSize), Number(pageSize)] : []
  return pageSize ? [Number(pageSize), (Number(pageNum) - 1) * Number(pageSize)] : []
}
...
let sqlFunKey = (rows = {}, sqlarr = []) => {
  let sqlString = ''
  let value = []
  if (Object.values(rows).length === 0) return { sqlString, value }
  for (let i in sqlarr) {
    let item = sqlarr[i]
