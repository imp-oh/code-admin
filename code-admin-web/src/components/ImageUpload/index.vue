<template>
  <div class="component-upload-image">
    <el-upload multiple :action="uploadImgUrl" list-type="picture-card" :on-success="handleUploadSuccess"
      ...
      :auto-upload="autoUpload" :on-change="onChange">
...
export default {
  props: {
    ...
    uploadImgUrl: {
      type: String,
      default: process.env.VUE_APP_BASE_API + "/common/upload" // 上传的图片服务器地址
    },
    // 是否在选取文件后立即进行上传
    autoUpload: {
      type: Boolean,
          ...
          this.fileList = list.map(item => {
            if (typeof item === "string") {
              if (item.indexOf(this.baseUrl) === -1) {
                item = { name: this.baseUrl + item, url: this.baseUrl + item, categoryImage: item };
              } else {
