            ...
            <table class="c-table align-center" border="0" cellpadding="0" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr v-for="(item, index) in form.tableData" :key="'list-' + index">
                        ...
                        <td style="width:100px">{{ item.actualBeginTime }}</td>
                        <td style="width:100px">{{ item.actualEndTime }}</td>
                        <td>{{ item.workDone ? item.workDone + '%' : "" }}</td>
                        <td>{{ item.jobRating }}</td>
                    </tr>
...
<script>
import { getViewPost } from "@/api/monthly/form";
...
export default {
    ...
    methods: {
        getList () {
            ...
            getViewPost(monthlyId).then(response => {
                this.form = response.data;
                this.form.yearMonth = formatTimes(this.form.yearMonth, 'yyyy-MM')
                console.log(this.form)
                if (this.form.tableData) {
                    this.form.tableData.forEach(item => {
                        item.beginTime = formatTimes(item.beginTime, 'yyyy-MM')
                        item.endTime = formatTimes(item.endTime, 'yyyy-MM')
                        item.actualBeginTime = formatTimes(item.actualBeginTime, 'yyyy-MM')
                        item.actualEndTime = formatTimes(item.actualEndTime, 'yyyy-MM')
                    })
                    this.handleChange()
                }
